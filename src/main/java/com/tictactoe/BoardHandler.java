package com.tictactoe;

import java.util.ArrayList;

public class BoardHandler {
    public static boolean isFieldOccupied(String[][] situation, Field field) {
        if(isItOnBoardRange(situation, field)) {
            return !situation[field.y()][field.x()].equals("x") && (!situation[field.y()][field.x()].equals("o"));
        } else {
            return false;
        }
    }

    public static boolean isItOnBoardRange(String[][] situation, Field field) {
        try{
            String temp = situation[field.y()][field.x()];
            return true;
        } catch(ArrayIndexOutOfBoundsException e) {
            return false;
        }
    }

    public static Field getFieldByNr(String[][] situation, int fieldIndex) {
        for(int i=0; i< situation.length; i++) {
            for(int j=0; j<situation[i].length; j++) {
                if(situation[i][j].equals(Integer.toString(fieldIndex)) || situation[i][j].equals("0"+ fieldIndex)) {
                    return new Field(i,j);
                }
            }
        }
        return new Field(-1,-1);
    }

    public static String getNrByField(State state, Field field) {
        int fieldNr = 0;
        for(int i=0; i<state.getSize(); i++) {
            for(int j=0; j< state.getSize(); j++) {
                if(i==field.y() && j==field.x()){
                    return Integer.toString(fieldNr);
                }
                fieldNr++;
            }
        }
        return null;
    }

    public static ArrayList<Field> buildFieldArray(String[][] situation) {
        ArrayList<Field> fields = new ArrayList<>();
        for(int i=0; i<situation.length; i++){
            for(int j=0; j<situation[i].length; j++){
                if(!situation[i][j].equals("x") && !situation[i][j].equals("o")){
                    fields.add(new Field(i,j));
                }
            }
        }
        return fields;
    }

    public static boolean doesEmptyFieldExists(String[][] situation) {
        ArrayList<Field> fields = buildFieldArray(situation);
        return !fields.isEmpty();
    }
}
